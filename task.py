...
class TravelTasks:
    ...
    def plan_itinerary(self, agent: str, city: str, travel_dates: str, interests: str):
        return Task(
            description=dedent(
                f"""
            ...
            expected_output=dedent(
                f"""
                Type: itinerary
                Description: A detailed 7-day travel itinerary, including daily plans, weather forecasts, places to eat, packing suggestions, and a budget breakdown.
                """
            )
        )

    def identify_city(self, agent: str, origin: str, cities: str, interests: str, travel_dates: str):
        return Task(
            description=dedent(
                f"""
            ...
            expected_output=dedent(
                f"""
                Type: city_selection
                Description: A detailed report on the best city to visit, including flight costs, weather forecast, attractions, and overall travel expenses.
                """
            )
        )

    def gather_city_info(self, agent: str, city: str, travel_dates: str, interests: str):
        return Task(
            description=dedent(
                f"""
            ...
            expected_output=dedent(
                f"""
                Type: city_info
                Description: An in-depth city guide with information on key attractions, local customs, special events, daily activity recommendations, weather forecasts, and high-level costs.
                """
            )
        )
